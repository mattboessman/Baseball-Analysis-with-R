...
for (m in 5:8){
  ...
  for (d in 01:30) {
    ...
    awaypitall_data[[idx]]<- df
    
    idx <- idx + 1
  }
  
  awaypitall_data <- lapply(awaypitall_data, function(x)
...
awaypitfinal_data <-bind_rows(awaypitall_data)

write_xlsx(awaypitfinal_data, "awaypitfinaldata.xlsx")
